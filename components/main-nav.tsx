...
import { LogoStandaloneIcon, SquareArrowTopRightIcon } from "@fingertip/icons";
      ...
      <ContextMenu>
        <ContextMenuTrigger asChild>
          <Link href="/" className="relative mr-6 flex items-center space-x-2">
            <LogoStandaloneIcon className="size-6" />
        ...
        <ContextMenuContent>
          ...
          >
            <LogoStandaloneIcon className="size-4" />
      ...
      <nav className="hidden items-center space-x-6 text-sm font-medium xl:flex">
        {docsConfig.mainNav.map((item) => (
          ...
          >
            ...
            {item.external && (
              <SquareArrowTopRightIcon className="ml-2 size-4" />
            )}
          </Link>
        ))}
      </nav>
