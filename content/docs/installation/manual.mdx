...
[Follow the Tailwind CSS installation instructions to get started.](https://tailwindcss.com/docs/installation/framework-guides)
...
```bash
npm install tailwindcss-animate class-variance-authority cn
```

### Add icon library

If you're using the `default` style, install `blode-icons-react`:

```bash
npm install blode-icons-react
```

If you're using the `new-york` style, install `blode-icons-react`:
...
[`cn`](https://github.com/shadcn-ui/cn) does the conditional joining and the Tailwind conflict
resolution in one function, so it replaces `clsx` and `tailwind-merge`.
