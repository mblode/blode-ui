...
```bash
npx shadcn@latest add "https://ui.fingertip.com/r/styles/default/button"
```
...
```tsx
<Link className={buttonVariants({ variant: "destruct" })}>Click here</Link>
```
...
<ComponentPreview name="button-muted" description="A muted button" />
