...
Set the `loading` prop to render a centred spinner overlay while preserving the button's width. Loading buttons are automatically `disabled` and receive `aria-busy="true"`. The `asChild` prop is ignored while `loading` is true so the spinner overlay can render inside a real `<button>`.
